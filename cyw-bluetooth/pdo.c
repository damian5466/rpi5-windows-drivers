...
static NTSTATUS PdoSetForwardingStack(WDFDEVICE Child, WDFDEVICE Parent)
{
    PFDO_EXTENSION c = FdoGetExtension(Parent);
    PDEVICE_OBJECT target = WdfIoTargetWdmGetTargetDeviceObject(c->IoTargetSerial);
    PDEVICE_OBJECT child = WdfDeviceWdmGetDeviceObject(Child);
    if (!target || target->StackSize < 1 || target->StackSize > 124)
        return STATUS_DEVICE_CONFIGURATION_ERROR;
    // This PDO routes writes to a separate UART stack. Reserve that stack
    // plus the PDO and parent forwarding locations before BthMini attaches.
    // Otherwise its IRPs are too small to forward (REQUEST_NOT_ACCEPTED).
    child->StackSize = (CCHAR)(target->StackSize + 2);
    if (child->AlignmentRequirement < target->AlignmentRequirement)
        child->AlignmentRequirement = target->AlignmentRequirement;
...
--*/
{
    ...
    Status =  RtlUnicodeStringPrintf(&Buffer, L"%02u", _SerialNo);
    if (!NT_SUCCESS(Status)) {
        return Status;
    }
    ...
    Status = PdoSetForwardingStack(ChildDevice, _Device);
    if (!NT_SUCCESS(Status)) { WdfObjectDelete(ChildDevice); return Status; }
...
--*/
{
    ...
    // precedence over the DeviceDesc from the INF file.
    //
    Status = RtlUnicodeStringPrintf(&Buffer, L"cywbtserialbus_%02u", _SerialNo );
    if (!NT_SUCCESS(Status)) {
        goto Cleanup;
    }
