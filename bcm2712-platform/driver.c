...
/* Only the two exact project HIDs are allowed; no generic MMIO driver. */
static NTSTATUS GetKind(PWDFDEVICE_INIT init, unsigned *kind)
{
    WCHAR ids[256];
    ULONG used = 0;
    size_t at = 0, end, count;
    NTSTATUS status = WdfFdoInitQueryProperty(init, DevicePropertyHardwareID,
                                            sizeof(ids), ids, &used);
    if (!NT_SUCCESS(status)) return status;
    ...
    while (at < count && ids[at]) {
        UNICODE_STRING item, rng, thermal, rngVendor, thermalVendor;
        ...
        RtlInitUnicodeString(&rng, L"ACPI\\RPI1014");
        RtlInitUnicodeString(&thermal, L"ACPI\\RPI1015");
        RtlInitUnicodeString(&rngVendor, L"ACPI\\VEN_RPI&DEV_1014");
        RtlInitUnicodeString(&thermalVendor, L"ACPI\\VEN_RPI&DEV_1015");
        if (RtlEqualUnicodeString(&item, &rng, TRUE) ||
            RtlEqualUnicodeString(&item, &rngVendor, TRUE)) {
            *kind = PI5_KIND_RNG;
            return STATUS_SUCCESS;
        }
        if (RtlEqualUnicodeString(&item, &thermal, TRUE) ||
            RtlEqualUnicodeString(&item, &thermalVendor, TRUE)) {
            *kind = PI5_KIND_THERMAL;
            return STATUS_SUCCESS;
        }
        at = end + 1;
    }
